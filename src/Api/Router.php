...
class Router
{
    ...
    public function scan_apis()
    {
         ...
         */
        \do_action('a!yabe/ukiyo/api/router:before_scan', $finder);
        ...
        foreach ($declared_classes as $declared_class) {
            ...
            if (!$reflector->isSubclassOf(\Yabe\Ukiyo\Api\ApiInterface::class)) {
                continue;
            }
    ...
    public function register_apis() : void
    {
        ...
        $apis = \apply_filters('f!yabe/ukiyo/api/router:register_apis', $this->apis);
        foreach ($apis as $api) {
