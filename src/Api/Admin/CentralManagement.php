 ...
 * @since 1.0.0
 */
class CentralManagement extends AbstractApi implements ApiInterface
{
    ...
    {
        return 'admin/settings/central_management';
    }
    ...
    private function check_connection(WP_REST_Request $wprestRequest) : WP_REST_Response
    {
        ...
        $url = $payload['url'];
        $resp = \wp_remote_get(\sprintf('%s/wp-json', $url));
        if (\is_wp_error($resp)) {
    ...
    private function index(WP_REST_Request $wprestRequest) : WP_REST_Response
    {
        ...
        if ($search) {
            ...
            $where_clause[] = $wpdb->prepare("( c.title LIKE %s OR c.url LIKE )", $escaped_search, $escaped_search, $escaped_search);
        }
        $where_clause = $where_clause !== [] ? 'WHERE ' . \implode(' AND ', $where_clause) : '';
        $result = $wpdb->get_results(
            //phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
            $wpdb->prepare("SELECT c.* FROM {$wpdb->prefix}{$wpdb->yabe_ukiyo_prefix}_central c {$where_clause} LIMIT %d OFFSET %d", $per_page, $offset)
        );
        foreach ($result as $row) {
            $items[] = ['id' => (int) $row->id, 'status' => (bool) $row->status, 'title' => $row->title, 'url' => $row->url, 'secret_key' => $row->secret_key, 'created_at' => (int) $row->created_at, 'updated_at' => (int) $row->updated_at];
        }
        $total_exists = (int) $wpdb->get_var("\n            SELECT COUNT(*)\n            FROM {$wpdb->prefix}{$wpdb->yabe_ukiyo_prefix}_central c\n        ");
        //phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
        $total_filtered = (int) $wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->prefix}{$wpdb->yabe_ukiyo_prefix}_central c {$where_clause}");
    ...
    private function store(WP_REST_Request $wprestRequest) : WP_REST_Response
    {
        ...
        $url = $payload['url'];
        $wpdb->insert(\sprintf('%s%s_central', $wpdb->prefix, $wpdb->yabe_ukiyo_prefix), ['uid' => Common::random_slug(10), 'title' => $title, 'secret_key' => $secret_key, 'url' => $url, 'status' => $status, 'created_at' => \time(), 'updated_at' => \time()], ['%s', '%s', '%s', '%s', '%d', '%d', '%d']);
        $id = $wpdb->insert_id;
        \do_action('a!yabe/ukiyo/api/admin/central_management:store', $id);
    ...
    private function update_status(WP_REST_Request $wprestRequest) : WP_REST_Response
    {
        ...
        $wpdb->update(\sprintf('%s%s_central', $wpdb->prefix, $wpdb->yabe_ukiyo_prefix), ['status' => $status], ['id' => $id], ['%d'], ['%d']);
        \do_action('a!yabe/ukiyo/api/admin/central_management:update_status', $id);
    ...
    private function destroy(WP_REST_Request $wprestRequest) : WP_REST_Response
    {
        ...
        $wpdb->delete(\sprintf('%s%s_central', $wpdb->prefix, $wpdb->yabe_ukiyo_prefix), ['id' => $id], ['%d']);
        \do_action('a!yabe/ukiyo/api/admin/central_management:destroy', $item);
    ...
    private function detail(WP_REST_Request $wprestRequest) : WP_REST_Response
    {
        ...
        $payload = ['id' => (int) $row->id, 'status' => (bool) $row->status, 'secret_key' => $row->secret_key, 'title' => $row->title, 'url' => $row->url, 'created_at' => (int) $row->created_at, 'updated_at' => (int) $row->updated_at];
    ...
    private function update(WP_REST_Request $wprestRequest) : WP_REST_Response
    {
        ...
        $url = $payload['url'];
        $status = (bool) $payload['status'];
        $wpdb->update(\sprintf('%s%s_central', $wpdb->prefix, $wpdb->yabe_ukiyo_prefix), ['title' => $title, 'secret_key' => $secret_key, 'url' => $url, 'status' => $status, 'updated_at' => \time()], ['id' => $id], ['%s', '%s', '%s', '%d', '%d'], ['%d']);
        \do_action('a!yabe/ukiyo/api/admin/central_management:update', $id);
        return new WP_REST_Response(['id' => $id], 200, []);
    }
}
